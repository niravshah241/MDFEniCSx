...
"""
# Coefficients of camber-line-abscissa parameter equation
c_1, c_2 = 0.5, 0.5
# Coefficients of camber-line.ordinate parameter equation
c_3, c_4 = 0.1, 0.05
# Coefficients of the thickness equation taken from the NACA 2412 Airfoil
coeff_thick = [0.2969, -0.126, -0.3516, 0.2843, -0.1036]

# Bézier curves with control parameter k \in [0,1]


def x_c(k): return 3 * c_1 * k * (1 - k)**2 + 3 * c_2 * \
    (1-k) * k**2 + k**3    # Camber-line-abscissa


def y_c(k): return 3 * c_3 * k * (1 - k)**2 + 3 * c_4 * \
    (1-k) * k**2           # Camber-line-abscissa

# Thickness equation for a 6% thick airfoil


def thickness(x):
    return 0.06 / 0.1 * (coeff_thick[0] * np.sqrt(x) + coeff_thick[1] * x +
                         coeff_thick[2] * x**2 + coeff_thick[3] * x**3 +
                         coeff_thick[4] * x**4)


# Position of the airfoil in the computational domain
# defined by the coordinates of the leading edge
leading_edge_x = np.min([L/8, 0.5])
leading_edge_y = H/2

# Upper and lower surface of the airfoil
x_u = x_l = lambda k: leading_edge_x + x_c(k)
def y_u(k): return leading_edge_y + y_c(k) + thickness(x_c(k))
def y_l(k): return leading_edge_y + y_c(k) - thickness(x_c(k))


# Calculate maximal thickness of the airfoil
thickness_max = np.max([thickness(x)
                       for x in np.linspace(0, 1, num_airfoil_refinement)])
...
gmsh.model.mesh.field.setNumbers(distance_field, "PointsList", [
                                 points_lower_curve[0],
                                 points_lower_curve[-1]])
gmsh.model.mesh.field.setNumber(
    distance_field, "Sampling", num_airfoil_refinement*2)
...
# (LcMin for the mesh size in the refined region,
# LcMax for the mesh size far from the refined region)
...
mesh, cell_tags, facet_tags = dolfinx.io.gmshio.read_from_msh(
    os.path.join(dir, "mesh.msh"), mesh_comm, gmsh_model_rank, gdim=gdim)
